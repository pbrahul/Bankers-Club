...
Public Class FrmSalesReturnPayment
    ...
    Private Sub FrmCustomer_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblPaidTK.Text = FrmSalesReturn.lblReceive.Text
        txtChange.Text = Format(0 - (CDbl(GrandTotal) - CDbl(lblPaidTK.Text)), "###,###.00")
        lblPaid.Visible = True
        lblPaidTK.Visible = True
    ...
    Private Sub txtReceive_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtReceive.TextChanged
        ...
        Else
            txtChange.Text = Format(CDbl(txtReceive.Text) - (CDbl(GrandTotal) - CDbl(lblPaidTK.Text)), "###,###.00")
            ChangeTK = txtChange.Text
        End If
    ...
    Private Sub btnOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        With FrmSalesReturn
            ...
            sqlSTR = "INSERT INTO tbl_Sales(Sales_ID, Sales_Date, Void, Sub_Total, Total_Vat, Total_Discount, Grand_Total, Receive_Amount, Change_Amount, Disount_Amount, Cust_ID, Order_No, User_ID) " & _
                     "VALUES ('" & GetID & "', '" & Format(Now, "yyyy/MM/dd") & "', 'No', " & CDbl(.lblSubTotal.Text) & ", " & CDbl(.lblVat.Text) & ", " & CDbl(.lblDiscount.Text) & ", " & CDbl(GrandTotal) - CDbl(txtDiscount.Text) & ", " & CDbl(txtReceive.Text) & _
            ...
            For Each dgv As DataGridViewRow In .dgvSales.Rows
                ...
                sqlSTR = "INSERT INTO tbl_Sales_Details(Sales_Details_ID, Sales_ID, Barcode, Product_Name, Qty, Sales_Price, Discount_Amount, Vat_Amount, Sub_Total) " & _
                         "VALUES ('" & SalesDetailsID & "', '" & GetID & "', '" & dgv.Cells(0).Value & "', '" & dgv.Cells(1).Value & "', " & CDbl(dgv.Cells(2).Value) & _
                         ", " & CDbl(dgv.Cells(4).Value) & ", " & CDbl(dgv.Cells(5).Value) & ", " & CDbl(dgv.Cells(6).Value) & ", " & CDbl(dgv.Cells(7).Value) & ")"
                ExecuteSQLQuery(sqlSTR)
        ...
        '    With FrmSalesReturn
        '        sqlSTR = "SELECT Barcode, Qty FROM tbl_Sales_Details WHERE Sales_ID = '" & EditID & "'"
        '        ExecuteSQLQuery(sqlSTR)
        ...
        '        ExecuteSQLQuery("DELETE FROM tbl_Sales_Details WHERE Sales_ID = '" & EditID & "'")

        '        sqlSTR = "UPDATE tbl_Sales SET Void = 'Yes', Sub_Total = " & CDbl(.lblSubTotal.Text) & ", Total_Vat = " & CDbl(.lblVat.Text) & ", Total_Discount = " & CDbl(.lblDiscount.Text) & ", Grand_Total = " & CDbl(GrandTotal) & _
        '                 ", Receive_Amount = Receive_Amount +" & CDbl(txtReceive.Text) & ", Change_Amount = " & CDbl(txtChange.Text) & ", Disount_Amount = " & CDbl(txtDiscount.Text) & " WHERE Sales_ID = '" & EditID & "'"
        '        ExecuteSQLQuery(sqlSTR)
        ...
        '            sqlSTR = "INSERT INTO tbl_Sales_Details(Sales_Details_ID, Sales_ID, Barcode, Product_Name, Qty, Sales_Price, Discount_Amount, Vat_Amount, Sub_Total) " & _
        '                     "VALUES ('" & SalesDetailsID & "', '" & EditID & "', '" & dgv.Cells(0).Value & "', '" & dgv.Cells(1).Value & "', " & CDbl(dgv.Cells(2).Value) & _
        '                     ", " & CDbl(dgv.Cells(3).Value) & ", " & CDbl(dgv.Cells(4).Value) & ", " & CDbl(dgv.Cells(5).Value) & ", " & CDbl(dgv.Cells(6).Value) & ")"
        '            ExecuteSQLQuery(sqlSTR)
